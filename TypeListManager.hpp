		...
		namespace ApplicationSystem{
			...
			class TypeListManager : virtual public TypePointerListManager<A,Args...>{
				public:
					...
					TypeListManager(Base::InvokeMethod<Base::Type&> a_add, Base::InvokeMethod<Base::Type&> a_remove, Base::InvokeMethod<Base::Type&,A&> a_initialize, Base::InvokeMethod<Base::Type&,A&> a_update) : TypePointerListManager<A,Args...>(a_add, a_remove, a_initialize, a_update){
						TypeListManagerLog(pankey_Log_StartMethod, "Constructor", "");
						TypeListManagerLog(pankey_Log_EndMethod, "Constructor", "");
					}
					...
					template<class AS>
					void put(){
						TypeListManagerLog(pankey_Log_StartMethod, "put",  "");
						if(this->containByIsType(ClassType<AS>::getId())){
							return;
						}
						
						TypeListManagerLog(pankey_Log_EndMethod, "put", "");
					}

					// template<class AS>
					// void put(InvokeMethod<AS&> a_method){
					// 	TypeListManagerLog(pankey_Log_StartMethod, "put",  "");
					// 	if(this->containByIsType(ClassType<AS>::getId())){
					// 		return;
					// 	}
					// 	AS& i_instance = (AS&)*this->addTypePointer(new AS());
					// 	invoke<AS&>(a_method, i_instance);
					// 	TypeListManagerLog(pankey_Log_EndMethod, "put", "");
					// }
