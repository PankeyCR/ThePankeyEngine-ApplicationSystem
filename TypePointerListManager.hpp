		...
		namespace ApplicationSystem{
			...
			class TypePointerListManager : virtual public UpdateManager<A,Args...>{
				public:
					...
					TypePointerListManager(Base::InvokeMethod<Base::Type&> a_add, Base::InvokeMethod<Base::Type&> a_remove, Base::InvokeMethod<Base::Type&,A&> a_initialize, Base::InvokeMethod<Base::Type&,A&> a_update){
						TypePointerListManagerLog(pankey_Log_StartMethod, "Constructor", "");
						m_add = a_add;
						m_remove = a_remove;
					...
					virtual bool containByType(long a_type){
						...
						for(int x = 0; x < m_initialize_type_list.length(); x++){
							Base::Type* f_type = m_initialize_type_list.getPointerByIndex(x);
							if(f_type == nullptr){
								continue;
							}
						...
						for(int x = 0; x < m_type_list.length(); x++){
							Base::Type* f_type = m_type_list.getPointerByIndex(x);
							if(f_type == nullptr){
								continue;
							}
							if(f_type->getType() == a_type){
					...
					virtual bool containByTypePointer(Base::Type* a_type){
					...
					virtual Base::Type* getTypePointerByType(long a_type){
					...
					virtual Base::Type* removeTypePointer(Base::Type* a_type){
						TypePointerListManagerLog(pankey_Log_StartMethod, "removeTypePointer",  "");
						Base::Type* i_type = m_initialize_type_list.removePointerByPointer(a_type);
						if(i_type == nullptr){
						...
						Base::invoke<Base::Type&>(m_remove, *i_type);
						TypePointerListManagerLog(pankey_Log_EndMethod, "removeTypePointer", "");
						return i_type;
					}

					virtual Base::Type* removeTypePointerByType(long a_type){
					...
					virtual bool destroyByTypePointer(Base::Type* a_type){
						TypePointerListManagerLog(pankey_Log_StartMethod, "destroyByTypePointer",  "");
						Base::Type* i_type = this->removeTypePointer(a_type);
						if(i_type == nullptr){
							return false;
						}
						delete i_type;
					...
					virtual bool destroyByType(long a_type){
						...
						Base::Type* i_type = this->removeTypePointerByType(a_type);
						if(i_type == nullptr){
							return false;
						}
						delete i_type;
					...
					virtual int TypePointer_length()const{
					...
					virtual void clearTypePointer(){
						...
						for(int x = 0; x < m_type_list.length(); x++){
							...
							Base::invoke<Base::Type&>(m_remove, *f_type);
						}
						for(int x = 0; x < m_initialize_type_list.length(); x++){
							Base::Type* f_type = m_initialize_type_list.getPointerByIndex(x);
							if(f_type == nullptr){
					...
					virtual void initialize(A& a_app){
						...
						for(int x = 0; x < this->m_initialize_type_list.length(); x++){
							...
							Base::Type* f_type = this->m_initialize_type_list.getPointerByIndex(x);
							if(f_type == nullptr){
								TypePointerListManagerLog(pankey_Log_StartMethod, "initialize",  "initializing state is null");
								continue;
							}
							Base::invoke<Base::Type&,A&>(m_initialize, *f_type, a_app);
					...
					virtual void update(A& a_app, Args... a_values){
						...
						for(int x = 0; x < this->m_type_list.length(); x++){
							...
							Base::Type* f_type = this->m_type_list.getPointerByIndex(x);

							if(f_type == nullptr){
								continue;
							}

							Base::invoke<Base::Type&,A&>(m_update, *f_type, a_app);
						}
				...
				protected:
					...
					Base::ArrayRawPointerList<Base::Type> m_type_list;
					Base::ArrayRawPointerList<Base::Type> m_initialize_type_list;
					
					Base::InvokeMethod<Base::Type&,A&> m_initialize = nullptr;
					Base::InvokeMethod<Base::Type&,A&> m_update = nullptr;
					
					Base::InvokeMethod<Base::Type&> m_add = nullptr;
					Base::InvokeMethod<Base::Type&> m_remove = nullptr;
			};
